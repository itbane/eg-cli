...
def calculateRoute(sX,sY,tX,tY,verbose):
  printVerbose("Calculating route from "+str(sX)+":"+str(sY)+" to "+str(tX)+":"+str(tY),verbose)
  (orderlist,operation_duration, operation, shortestpath) = dijkstra.dijkstra(int(sX),int(sY),int(tX),int(tY),0)
  return orderlist
...
def travel(source,target,verbose,add,list,create):
  sX,sY = splitCoord(source)
  tX,tY = splitCoord(target)
  orderlist = calculateRoute(sX,sY,tX,tY,verbose)
  if list:
